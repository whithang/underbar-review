(function() {
  ...
  _.reject = function(collection, test) {
    ...
    for (var i = 0; i < collection.length; i++) {
      for (var j = 0; j < trueCollection.length; j++) {
        if (collection[i] === trueCollection[j]) {
          doesContain = true;
        }
      }
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (prevArguments.hasOwnProperty(argList)) {
        result = prevArguments[argList];
      } else {
        result = func.apply(this, arguments);
        prevArguments[argList] = result;
      }
      return result;
    };
  };
  ...
  _.shuffle = function(array) {
    ...
    while (shuffleArray.length > 0) {
      loc = Math.floor(Math.random() * (shuffleArray.length - 1));
      newArray = newArray.concat(shuffleArray.splice(loc, 1));
    }
